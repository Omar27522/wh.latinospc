<?php
// views/product_card_edit.php
// Modular component for inline product editing by warehouse administrators
// $p is passed in containing product attributes
?>
<article class='card edit-mode-card'>
    <!-- Top Bar: Origin Badge & Admin Actions (No overlapping) -->
    <div class="card-top-bar">
        <div>
            <?php if (!empty($p['location_code']) && !empty($p['is_warehouse'])): ?>
                <span class="card-origin-badge" title="Imported from Warehouse Physical Shelf">📍 <?= htmlspecialchars($p['location_code']) ?></span>
            <?php else: ?>
                <span class="card-origin-badge custom" title="Custom store item">🏪 Custom</span>
            <?php endif; ?>
        </div>

        <div class="card-admin-floating-actions">
            <?php if (!empty($p['is_warehouse'])): ?>
                <form method='POST' action='admin_action.php' onsubmit='return confirm("Unpost this product from the storefront? It will remain safely stored in warehouse inventory.")' style="margin: 0;">
            ...
            <form method='POST' action='admin_action.php' onsubmit='return confirm("Delete this item permanently?")' style="margin: 0;">
